...
def apply_retention(storage: SFTPStorage, policy: RetentionPolicy, dry_run: bool = False) -> int:
    """
    ...
    Args:
        ...
        dry_run: If True, log deletions but not execute them.

    Returns:
        Number of files deleted (or that would be deleted in dry_run mode).
    """
    logger.info("Starting retention run (try_run=%s)...", dry_run)
    ...
    for f in sorted(to_delete):
        if dry_run:
            logger.info("[DRY RUN] Would dlete: %s", f.path)
        else:
            try:
